...
* 可以用常量或表达式对引用进行初始化，但此时必须用`const`作声明(<font color=  blue>内部是使用一个`temp`临时变量转换)；
...
* 系统也会提供一个默认的拷贝构造函数，默认是浅拷贝，形如`Clazz c1(c2)`的使用，将`c1`拷贝给`c2`；
...
```
### 常对象
* 可以在定义对象时加上关键字`const`，指定对象为常对象。常对象必须要有初值，凡是希望保护数据成员不被改变的对象，都可以声明为常对象。
* 基本语法: `类名 const 对象名 [(实参表)]` 或者`const 类名 对象名 [(实参表)]`，举例: `Time const t(10, 10, 30)`和`const Time t(10, 10, 30)`；
* 如果一个对象被声明为常对象，则通过改对象只能调用它的常成员函数，不能调用对象的普通成员函数；例如`void get_time() const` (常成员函数)；
...
②常成员函数
* 常成员函数声明: 例如`void get_time() const`，常成员函数只能引用本类中的数据成员，不能修改它们；
...
char *p2 = c; // err
```
* `const`指针除了可以指向`const`变量，还可以指向非`const`变量，此时不能通过指针来改变变量的值；
...
②指向常对象的指针
* 和指向常变量的指针类似，只能用指向常对象的指针变量指向它，不能用指向非`const`对象的指针指向它；
* 如果定义了一个指向常对象的指针变量，并指向了一个非`const`对象，则不能通过改指针变量来改变变量的值；
...
* 静态数据成员只能在类体外进行初始化。如`int Box::height = 10;`，只在类体中声明静态数据成员时加上`static`，不必在初始化语句中加`static`。
* 不能在参数初始化列表中对静态数据成员进行初始化；

②静态成员函数
* 静态成员函数和普通函数最大的区别就在静态成员函数没有`this`指针，决定了静态成员函数与不能访问本类中的非静态成员。所以静态成员函数的作用就是用来访问静态数据成员；
...
* 注意: 重载这两个运算符的时候，只能将他们作为友元函数，不能作为成员函数。避免修改 `c++`的标准库。
...
```
运行结果: 
![image](https://picsur.cloud.fairies.ltd/i/69c13d16-7e62-4968-80bf-8e2acbcecb14.webp)
...
* 要注意；
* 读和写是站在应用程序的角度来看的；
![image](https://picsur.cloud.fairies.ltd/i/2e07e2c5-f562-44ef-ab61-f2c80fbc92c1.webp)
...
```
运行效果: 
[img]https://picsur.cloud.fairies.ltd/i/34f295a1-9e86-4a99-b6ff-e2c0387fa43f.webp[/img]
...
```
运行结果：　
![image](https://picsur.cloud.fairies.ltd/i/d6456e7a-8e27-4fc2-a720-b243b3862d45.webp)
...
![image](https://picsur.cloud.fairies.ltd/i/926beb83-0552-4f5c-ae9e-de0dcace7cea.webp)
...
演示结果: 
![image](https://picsur.cloud.fairies.ltd/i/b4b07d41-d507-4a66-8feb-a4a8ae609ca2.webp)
